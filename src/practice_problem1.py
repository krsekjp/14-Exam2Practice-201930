...
class Box(object):
    ...
    def __init__(self, contents, volume):
        ...
        if len(contents) <= volume:
            ...
            self.contents = ''
        self.volume_init = self.volume
        self.contents_init = self.contents
        self.reset_info = []
    ...
    def append_string(self, additional_contents):
        ...
        # ---------------------------------------------------------------------
        full_string = self.contents + additional_contents
        if len(full_string) > self.volume:
            s = full_string[self.volume:]
            self.contents = full_string[:self.volume]
            return s
        else:
            self.contents = full_string
            return ''
    ...
    def reset(self):
        ...
        #    TIME ESTIMATE:   5 minutes.
        # ---------------------------------------------------------------------
        self.reset_info = self.reset_info + [self.contents]
        self.volume = self.volume_init
        self.contents = self.contents_init
    ...
    def combined_box(self, other_box):
        ...
        #    TIME ESTIMATE:   5 minutes.
        # ---------------------------------------------------------------------
        combo_box = Box(self.contents+other_box.contents, self.volume+other_box.volume)
        return combo_box
